...
HAL_StatusTypeDef flash_update_generic(const FLASH_STORED_DATA *new)
{
	...
	if(ret == HAL_OK)
	{
		...
		if (ret == HAL_OK)
		{
			...
			{
				ret = HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, ((uint32_t) &FlashStoredData) + i, *(uint32_t*) (((uint32_t) new) + i)); // did you know FLASH_TYPEPROGRAM_DOUBLEWORD needs Vpp? now we both know
			}
		}
